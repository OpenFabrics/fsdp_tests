...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/submit">
    <job retention_tag="scratch">
      <whiteboard>
        <xsl:choose><xsl:when test="whiteboard = ''">RDMA environment setup for multihost tests
  ...
  <xsl:template match="/submit/recipe">
	...
	<task name="/kernel/infiniband/ofa-fsdp/env_setup" role="SERVERS">
      ...
      <recipe kernel_options="" kernel_options_post="" role="N2" whiteboard="2nd machine">
        <xsl:if test="contains(distro/@family, 'Linux8') or contains(distro/@family, 'Fedora')">
          <xsl:attribute name="ks_meta">harness=restraint-rhts</xsl:attribute>
        </xsl:if>
	<autopick random="false"/>
